...
use ctfs::core::{AdmitError, StateCell, TenantKernel};
...
fn main() {
    ...
    alice_cell
        .as_slice_mut()
        .copy_from_slice(alice_state.as_bytes());

    let mut bob_cell = StateCell::new();
    bob_cell
        .as_slice_mut()
        .copy_from_slice(bob_state.as_bytes());

    kernel
        .register_tenant(1, alice_cell)
    ...
    kernel
        .admit(1, &alice_fact1, &[], &alice_delta)
        .expect("alice fact1");
    ...
    kernel
        .admit(1, &alice_fact3, &[alice_fact1], &alice_delta)
        .expect("alice fact3");
    ...
    let alice_balance = kernel
        .state(1)
        .unwrap()
        .cast_ref::<ConservationState>()
        .unwrap()
        .balance;
    let bob_balance = kernel
        .state(2)
        .unwrap()
        .cast_ref::<ConservationState>()
        .unwrap()
